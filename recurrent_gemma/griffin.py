...
def rnn_scan(x, a, h0):
    ...
    if x.shape[1] == 1:
        ...
        if h0 is not None:
            ...
            h_t = mx.zeros(x[:, 0].shape, dtype=x.dtype)
...
class MLPBlock(nn.Module):
  ...
  def __init__(self, width: int, expanded_width: int):
      super().__init__()
      self.up_proj = nn.Linear(width, expanded_width)
      self.gate_proj = nn.Linear(width, expanded_width)
      self.down_proj = nn.Linear(expanded_width, width)
  ...
  def __init__(
    ...
    if self.temporal_block_type == "recurrent":
        self.temporal_block = RecurrentBlock(
            width=self.width,
            num_heads=self.num_heads,
            lru_width=self.lru_width, # TODO use that?
        ...
        self.temporal_block = LocalAttentionBlock(
            width=self.width,
            num_heads=self.num_heads,
            window_size=self.attention_window_size, # TODO use that?
        )
